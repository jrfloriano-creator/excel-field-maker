...
export function PagarForm({ clienteNome, valorOriginal, creditoDisponivel = 0, funcionarios, formasPagamento = [], onSubmit, onClose }: PagarFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isValid = await verifyPin(pin, func.pin);
    if (!isValid) {
      toast.error('Senha do funcionário incorreta — pagamento não confirmado');
      return;
    }
