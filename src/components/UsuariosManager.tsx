...
export function UsuariosManager({ config, onUpdate }: Props) {
  const usuarios = ensureMasterUser(config.usuarios);
  ...
  const addUsuario = () => {
    ...
    const novo: Usuario = { id: generateId(), nome: nome.trim(), pin: hashPin(pin), nivel };
  ...
  const salvarSenha = () => {
    ...
    onUpdate({
      usuarios: usuarios.map(u => u.id === editPin.id ? { ...u, pin: hashPin(editPin.valor) } : u),
    });
    setEditPin(null);
