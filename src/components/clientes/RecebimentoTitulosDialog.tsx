...
export function RecebimentoTitulosDialog({ cliente, titulos, config, user, updateTitulo, updateConfig, onClose }: Props) {
  ...
  const totalOriginal = useMemo(
    () => titulosSelecionados.reduce((s, t) => s + t.valorCorrigido, 0),
    [titulosSelecionados]
  );
  ...
  const handleConfirmar = async () => {
    ...
    try {
      ...
      titulosSelecionados.forEach((t, idx) => {
        ...
        const proporcional = totalOriginal > 0 ? (t.valorCorrigido / totalOriginal) * valorRecebidoN : 0;
      ...
      appendLog(
        ...
        user,
        'titulo.pagar',
        `Recebeu ${titulosSelecionados.length} título(s) de ${cliente?.nome} — ${formatCurrency(valorRecebidoN)} via ${formaSel.nome} por ${usuarioSel.nome}`
      );
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-muted-foreground">
                      Venc.: {formatDate(t.vencimento)} • {formatCurrency(t.valorCorrigido)}
                      {t.valorJuros > 0 && ` (juros ${formatCurrency(t.valorJuros)})`}
                    </p>
                  </div>
                </label>
              ))}
